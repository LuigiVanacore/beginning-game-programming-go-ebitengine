...
func registerPlayerInput(inp *InputManager) {
	...
	// Ch12: Escape leaves gameplay for the pause menu (handled by the state machine).
	inp.AddAction(ActionPause, NewKeyRawInputButton(ebiten.KeyEscape))
}
...
func wirePlayerCallbacks(player *Player, game *Game) {
	...
	player.SetWeaponHit(func(proj, enemyCol *Collider, dmg float64) {
		...
		// A weapon connected with an enemy: sound + blood + a floating damage number.
		// TakeDamage also triggers the enemy's white hit flash.
		Audio().Play(SoundHit)
		pos := enemy.GetWorldPosition()
		if game.particles != nil {
			game.particles.EmitBloodSmall(pos.X(), pos.Y())
		}
		if game.floatingText != nil {
			game.floatingText.AddDamage(pos.X(), pos.Y(), dmg)
		}
		enemy.TakeDamage(dmg)
		if enemy.IsDead() {
			// A kill gets a bigger blood burst and a short screen shake.
			if game.particles != nil {
				game.particles.EmitBlood(pos.X(), pos.Y())
			}
			game.engine.World().Camera().Shake(4)
	...
	player.Collider.SetOnCollide(func(other *Collider) {
		...
		if enemy := game.enemyManager.FindByCollider(other); enemy != nil {
			...
			// Contact fires every overlapping frame, so throttle the feedback.
			if randFloat64() < 0.25 {
				pp := game.player.GetWorldPosition()
				if game.particles != nil {
					game.particles.EmitBloodSmall(pp.X(), pp.Y())
				}
				game.engine.World().Camera().Shake(2.5)
			}
		}
	})
