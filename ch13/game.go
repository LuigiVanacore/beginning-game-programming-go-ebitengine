...
func (g *Game) Update() error {
	...
	g.particles.Update()
	g.floatingText.Update()

	return g.engine.Update()
}

// emitWeaponTrails spawns trail particles behind every active projectile and the
// orbiting Sacred Book. Called once per frame after the weapons advance (ch13).
func (g *Game) emitWeaponTrails() {
	if g.particles == nil {
		return
	}
	if g.weapons.KnifeUnlocked && g.weapons.Knife != nil {
		g.weapons.Knife.ForEachProjectile(g.particles.EmitProjectileTrail)
	}
	if g.weapons.FlyingAxeUnlocked && g.weapons.Axe != nil {
		g.weapons.Axe.ForEachProjectile(g.particles.EmitProjectileTrail)
	}
	if g.weapons.SacredBookUnlocked && g.weapons.SacredBook != nil {
		x, y, tvx, tvy := g.weapons.SacredBook.TrailSample()
		g.particles.EmitOrbitTrail(x, y, tvx, tvy)
	}
}
...
func (g *Game) checkLevelUp() {
	...
	g.player.XP -= need
	g.player.Level++
	Audio().Play(SoundLevelUp)
	if g.particles != nil {
		p := g.player.GetWorldPosition()
		g.particles.EmitLevelUp(p.X(), p.Y())
	}
...
// IsPausePressed reports whether the pause action fired this frame. The state machine
// polls it to leave the gameplay state, so the state types never import the input core.
func (g *Game) IsPausePressed() bool {
	if g == nil || g.engine == nil {
		return false
	}
	return g.engine.Input().IsActionPressed(ActionPause)
}
