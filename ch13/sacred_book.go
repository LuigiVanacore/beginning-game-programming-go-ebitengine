...
// TrailSample returns the book collider's world position and a unit tangential
// direction of its circular motion, so the caller can emit an orbit trail (ch13).
func (w *SacredBook) TrailSample() (x, y, tangentVx, tangentVy float64) {
	if w == nil || w.col == nil {
		return 0, 0, 0, 0
	}
	wp := w.col.GetWorldPosition()
	return wp.X(), wp.Y(), -math.Sin(w.angle), math.Cos(w.angle)
}
