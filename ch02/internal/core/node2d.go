...
// Node2D extends Node with spatial transform (position, rotation, scale, pivot).
// Use for sprites, cameras, and any object that has a place in 2D space.
type Node2D struct {
	Node
...
func (n *Node2D) SetTransform(t Transform) {
	n.localTransform = t
	n.MarkDirty()
}

func (n *Node2D) SetPosition(x, y float64) {
	n.localTransform.SetPosition(NewVector2D(x, y))
	n.MarkDirty()
}
func (n *Node2D) GetPosition() Vector2D { return n.localTransform.GetPosition() }

func (n *Node2D) SetRotation(r float64) {
	n.localTransform.SetRotation(r)
	n.MarkDirty()
}
func (n *Node2D) GetRotation() float64 { return n.localTransform.GetRotation() }

func (n *Node2D) SetScale(x, y float64) {
	n.localTransform.SetScale(x, y)
	n.MarkDirty()
}
...
func (n *Node2D) SetPivot(x, y float64) {
	n.localTransform.SetPivot(x, y)
	n.MarkDirty()
}
