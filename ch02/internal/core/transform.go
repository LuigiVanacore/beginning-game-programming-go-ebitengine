...
// Result: parent * child in transform space.
// The pivot comes from the child, since it is defined in the drawn node's local (texture) space.
func (t *Transform) Concat(other Transform) {
	...
	t.pivot.SetPosition(other.GetPivot())
}
